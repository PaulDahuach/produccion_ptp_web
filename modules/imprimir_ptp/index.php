<?php
/**
 * Reimpresión de PTP (plantilla de ruta de procesos) — reproduce el legacy "Rpt PTP"
 * (cabecera + subreporte "Rpt PTP_Procesos"). El PTP es la receta de procesos que se
 * copia a las órdenes en Definición ("Cargar PTP").
 * Uso: ?id=NUMPTP  (o sin id → buscador).
 */
...
if ($id <= 0) {
    ...
    module_head('Reimprimir PTP', 'bi-printer', '');
    ?>
    <div class="card"><div class="card-body">
      <label class="form-label">N° de PTP</label>
      <div class="input-group" style="max-width:340px">
        <input type="number" id="num" class="form-control" placeholder="Ej. 100815" autofocus>
        <button class="btn btn-primary" id="go"><i class="bi bi-printer me-1"></i>Abrir</button>
      </div>
      <p class="text-muted small mt-2">Se abre el PTP listo para imprimir.</p>
    </div></div>
    <?php
    module_foot('<script>
...
$h = db_row("SELECT P.NUMPTP, P.FDEPTP, P.DENPTP, P.OBSPTP, Ed.DENEDP, C.DENCLI, M.DENMAR
             ...
             WHERE P.NUMPTP = $id;");
if (!$h) { http_response_code(404); echo "PTP $id no encontrado."; exit; }

$procs = db_query("SELECT PP.ORDPTP, Prc.DENPRC, E.DENETA, CP.DENCDP, PP.PORPTP, PP.OBSPTP
...
<div class="hoja">
  <div class="titulo">PTP — RUTA DE PROCESOS</div>
  ...
  <div class="row g-3">
    ...
    <div class="col-4"><div class="campo"><div class="lbl">Fecha</div><div class="val"><?= h(to_disp_date($h['FDEPTP'])) ?></div></div></div>
    <div class="col-4"><div class="campo"><div class="lbl">Estado</div><div class="val"><?= h($h['DENEDP']) ?></div></div></div>
  </div>
  <div class="row g-2 mt-1">
    <div class="col-12"><div class="campo"><div class="lbl">Denominación</div><div class="val"><?= h($h['DENPTP']) ?></div></div></div>
  ...
  <table class="proc">
    ...
    <tbody>
      ...
      <tr><td><?= h($p['ORDPTP']) ?></td><td><?= h($p['DENPRC']) ?></td><td><?= h($p['DENETA']) ?></td><td><?= h($p['DENCDP']) ?></td><td><?= h($p['PORPTP']) ?></td><td><?= h($p['OBSPTP']) ?></td></tr>
      <?php endforeach; else: ?>
  ...
  <?php if (!empty($h['OBSPTP'])): ?>
  <div class="seccion">Observaciones</div>
  <div class="obs"><?= h($h['OBSPTP']) ?></div>
  <?php endif; ?>
</div>
</body>
</html>
