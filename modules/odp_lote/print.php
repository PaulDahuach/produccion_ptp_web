<?php
/**
 * Impresión — Consulta Órdenes de Proceso x Lote (reproduce "Rpt Consulta Ordenes
 * de Proceso x Lote"). Acepta los mismos filtros que la vista (etapa, desde, hasta, q),
 * agrupa por SECTOR con subtotales y total general. Apaisado, print-friendly.
 */
...
$rows = db_query(sql_base() . "\nWHERE " . implode(' AND ', $where) . "\nORDER BY E.CODETA, O.NUMODP;");

// Agrupar por SECTOR
$grupos = [];
foreach ($rows as $r) { $s = $r['SECTOR'] ?: '(sin sector)'; $grupos[$s][] = $r; }
...
<head>
<meta charset="UTF-8">
<title>Órdenes de Proceso x Lote</title>
...
<div class="hoja">
  ...
  <div class="meta"><?= count($rows) ?> registro(s)<?= $filtros ? ' — ' . h(implode('  |  ', $filtros)) : '' ?></div>

  <?php if (!$rows): ?>
    <p class="text-center text-muted">Sin registros para los filtros seleccionados.</p>
  <?php else: foreach ($grupos as $sector => $items): $sub = 0; foreach ($items as $i) $sub += (float)$i['CANTIDAD']; ?>
    <div class="sector"><?= h($sector) ?> — <?= count($items) ?> ítem(s)</div>
    <table class="lst">
      <thead><tr>
        <th>ODP</th><th>Cliente</th><th>Marca</th><th>Prenda</th><th>Proceso</th>
        <th>O.Corte</th><th>Cód.Art.</th><th>PTP</th><th class="num">Cant.</th>
        <th class="num">Días Rec.</th><th class="num">Días Def.</th><th class="num">Orden</th><th>Lote</th>
      </tr></thead>
      <tbody>
        <?php foreach ($items as $r): ?>
        <tr>
          <td><?= h($r['ODP']) ?></td><td><?= h($r['CLIENTE']) ?></td><td><?= h($r['MARCA']) ?></td>
          <td><?= h($r['PRENDA']) ?></td><td><?= h($r['PROCESO']) ?></td>
          <td><?= h($r['OCORTE']) ?></td><td><?= h($r['CARTICULO']) ?></td><td><?= h($r['PTP']) ?></td>
          <td class="num"><?= h(number_format((float)$r['CANTIDAD'], 0, ',', '.')) ?></td>
          <td class="num"><?= h($r['DIAS_REC']) ?></td><td class="num"><?= h($r['DIAS_DEF']) ?></td>
          <td class="num"><?= h($r['ORDEN']) ?></td><td><?= h($r['LOTE']) ?></td>
        </tr>
        <?php endforeach; ?>
        <tr class="subt"><td colspan="8" class="num">Subtotal <?= h($sector) ?></td><td class="num"><?= h(number_format($sub, 0, ',', '.')) ?></td><td colspan="4"></td></tr>
      </tbody>
    </table>
  <?php endforeach; ?>
    <div class="totg">TOTAL GENERAL: <?= h(number_format($totGral, 0, ',', '.')) ?> prendas — <?= count($rows) ?> ítems</div>
  <?php endif; ?>
</div>
</body>
</html>
